...
def drive_2(t):
    return drive_sub(t) + drive_sub(t - inter_spike)
    ...
    for y in range(points):
        try:
            ...
            for tp in range(timepoints):
                t = t_array[tp]
                i[tp] = drive_2(t)
                v[tp] = v[tp-1] + dt * (-v[tp-1] - u[tp-1] + drive_2(t))
                u[tp] = u[tp-1] + dt * (R * v[tp-1] - D * u[tp-1])

                if t >= delay-1 and v[tp] >= 0 and zero_time == None:
                    zero_time = t
                    break
            zero_offset[y, x] = zero_time
            zero_time = None
            for tp in range(timepoints):
                t = t_array[tp]
                i[tp] = drive_1(t)
                v[tp] = v[tp-1] + dt * (-v[tp-1] - u[tp-1] + drive_1(t))
                u[tp] = u[tp-1] + dt * (R * v[tp-1] - D * u[tp-1])
...
img = ax.imshow(zero_offset, origin="lower",
                ...
                aspect=D_length/R_length, cmap="magma")
...
plt.title("Difference in delay for two transient inputs")
plt.show()
