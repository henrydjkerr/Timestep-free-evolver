...
for spike in range(1, max_spikes+1):
    ...
    for line in infile:
        ...
        if abs(spike_list[-1][1][n] - spike_list[0][1][n]) > precision * 0.5:
            print("Uh oh")
        n += 1
    infile.close()
...
#1-spike baseline
thiscol = getcol(1)
plt.hlines(0, 0, graze_list_R[0], color = thiscol)
plt.hlines(0, graze_list_R[0], 6, color = fade(thiscol))
plt.scatter(graze_list_R[0], 0, color = thiscol, zorder = 100)
...
plt.yticks([0, -1, -2, -3])
if print_mode:
    plt.savefig("difference in speed.pdf")
    plt.savefig("difference in speed.png")
else:
    plt.show()
