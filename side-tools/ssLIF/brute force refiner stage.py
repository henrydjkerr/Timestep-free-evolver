"""
This just lets you import a bunch of solutions and rerun the root finding
...
infile = open("3 spike data proper.csv")
points_in = []
for line in infile:
    entry = line.strip("\n").split(",")
    points_in.append([])
    for value in entry:
        points_in[-1].append(float(value))
infile.close()
...
outfile = open("3 spike data refined.txt", "w")
n = 0
for start_point in points_in:
    print(n, time.time() - stopwatch)
    print(start_point)
    n += 1
    R = start_point[-1]
    guess = start_point[:-1]
    ...
    if soln.success and soln.x[0] > 0 and soln.x[1] > 0 and soln.x[2] > 0:
        results_x.append(R)
        results_y.append(soln.x[0])
        points_out.append((soln.x, R))
        line = ""
        for entry in soln.x:
            line += str(entry) + ","
        line += str(R) + "\n"
...
plt.figure()
plt.scatter(results_x, results_y, s=1)
plt.xlabel("$R$")
