...
def v_old(t):
    return (I * (2*p - 1) / (p**2 - q2)) \
           + beta * (part_v_old(A, a, t) - part_v_old(B, b, t))

def part_v_old(Z, z, t):
    coeff_cos = (2*p - beta - 1) / ( (p - beta)**2 - q2 )
    coeff_sin = (p**2 + q2 - p*(beta + 1) + beta) / ( (p - beta)**2 - q2 )
    coeff_sin /= abs_q

    part_p = -coeff_cos * calc_integral_old(z, t, p, "cosine")
    part_p += coeff_sin * calc_integral_old(z, t, p, "sine")

    part_beta = coeff_cos * calc_integral_old(z, t, beta, None)
    return Z * (part_p + part_beta)

def u(t):
    return C*I / (p**2 - q2) \
           + C * beta * (part_u_old(A, a, t) - part_u_old(B, b, t))
           
def part_u_old(Z, z, t):
...
def s_old(t):
    return beta * (part_s_old(A, a, t) - part_s_old(B, b, t))

def part_s_old(Z, z, t):
    return Z * calc_integral_old(z, t, beta, None)
    ...
    if t_old != None:
        ...
        part_init = e**(-p * (t - t_old))       \
                    * (v_r * cos(abs_q * (t - t_old))
                       - (v_r*(1 - p) + u_old) * sin(abs_q * (t - t_old)))
...
for n in range(steps):
    ...
    voltage[n] = v(t) - v_old(t)
    #wigglage[n] = u(t, t_last, u_last)
    wigglage[n] = u(t) - u_old(t)
    synapse[n] = s(t) - s_old(t)
