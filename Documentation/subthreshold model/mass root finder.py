...
results_x = []
results_y = []
for n in range(C_steps):
    print(n)
    update_C(n)
    for init_c in np.linspace(0.1, 3.1, x_count):
        for init_t in np.linspace(0.3, 8.5, y_count):
            guess = [init_c, init_t]
            soln = optimize.root(root_function, guess, tol=0.00002)
...
##counter = 0
##for init_c in np.linspace(0.1, 3.1, x_count):
##    for init_t in np.linspace(0.5, 8.5, y_count):
##        print(counter)
##        counter += 1
##        guess = [init_c, init_t]
##        for n in range(C_steps):
##            update_C(n)
##            soln = optimize.root(root_function, guess)
##            if soln.success and soln.x[0] > 0 and soln.x[1] > 0:
##                results_x.append(C)
##                results_y.append(soln.x[0])
##                guess = soln.x
...
plt.title("Wave solutions by speed for given $C$")
plt.show()
